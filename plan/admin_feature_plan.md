...
*   **디자인 컨셉: "The Burn Bag (소각장)"**
    *   **헤더:** "TOP SECRET" 스탬프는 유지하되, 페이지 제목은 **"보안 구역 (SECURE AREA)"**으로 표기.
    *   **언어:** "TOP SECRET"을 제외한 모든 UI 텍스트는 한국어로 번역 (예: "Authorized Personnel Only" -> "관계자 외 출입 금지").
    *   **리스트 뷰:** 
        *   기존 `LoadScenarioScreen`의 레이아웃을 재사용.
        *   **페이지네이션:** 한 페이지당 **10개**의 시나리오를 표시하고 하단에 페이지 이동 버튼 배치.
        *   **상세 정보:** 리스트 아이템에 사건 제목, 요약, 피해자, 용의자 목록, **진범(Culprit)** 정보를 모두 표시.
    *   **삭제 액션:**
        *   버튼 텍스트: "기록 말소".
        *   인터랙션: 삭제 시 붉은색 경고 모달 표시.
    *   **나가기:** "보안 구역 이탈" 버튼.
