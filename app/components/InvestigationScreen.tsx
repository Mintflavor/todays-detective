  ...
  return (
    <div className="flex flex-col h-screen bg-gray-950 text-gray-100 font-sans overflow-hidden relative">
      ...
      <div className="absolute inset-0 z-0">
        <Image
          src="/images/suspect_background.webp"
          alt="Suspect Background"
          fill
        ...
        <div className="w-full max-w-2xl flex flex-col space-y-4">
          {chatLogs[currentSuspectId].map((msg, idx) => (
            <div key={idx} className={`flex ${msg.role === 'user' || msg.role === 'note' ? 'justify-end' : msg.role === 'system' ? 'justify-center' : 'justify-start'} animate-fade-in`}>
              {msg.role === 'system' ? (
                ...
                <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed shadow-lg relative backdrop-blur-sm ${
                  msg.role === 'user' 
                    ? 'bg-amber-800/90 text-white rounded-tr-sm' 
                    : msg.role === 'note'
                      ? 'bg-gray-700/90 text-gray-300 rounded-tr-sm border border-gray-600 italic'
          ...
          {isTyping && (
              <div className="flex justify-start animate-pulse">
                <div className="bg-gray-800/80 backdrop-blur-sm text-gray-500 rounded-2xl rounded-tl-sm px-4 py-3 text-xs border border-gray-700">
                ...
                </div>
              </div>
          )}
      ...
      {/* Bottom Area */}
      <div className="bg-gray-900/95 backdrop-blur-sm border-t border-gray-800 z-20 shrink-0 pb-safe">
            ...
            <button
              ...
              className={`flex-1 py-3 px-2 text-sm font-medium transition-all relative min-w-[60px] ${
                currentSuspectId === 0 
                  ...
                  : 'bg-transparent text-gray-600 hover:bg-gray-800/30'
              }`}
            >
              ...
              <button
                ...
                className={`flex-1 py-3 px-2 text-sm font-medium transition-all relative min-w-[80px] ${
                  currentSuspectId === s.id 
                    ? 'bg-gray-800/50 text-amber-500' 
                    : 'bg-transparent text-gray-500 hover:bg-gray-800/30'
                }`}
              >
                <div className="flex flex-col items-center gap-1.5">
